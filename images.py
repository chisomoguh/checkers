...
if __name__ == '__main__':
    image = FileImage('anr.jpg')
    image.show('Anna Rafferty')
    grayscaleAnna = image.copyGray()
    grayscaleAnna.show('ANR, in gray', 125, 25)
    mutedAnna = image.copy()
    for x in range(mutedAnna.getWidth()):
        for y in range(mutedAnna.getHeight()):
            r, g, b = mutedAnna.getPixel(x, y)
            mutedAnna.setPixel(x, y, (r//2, g//2, b//2))
    mutedAnna.show('ANR, feeling muted', 250, 50)
